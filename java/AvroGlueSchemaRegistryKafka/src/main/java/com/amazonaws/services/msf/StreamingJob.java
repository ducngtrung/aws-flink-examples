...
/**
 * Sample Flink application that can run in Amazon Managed Service for Apache Flink (AMSAF)
 * It reads from a Kafka topic "temperature-samples" serialized as AVRO using AWS Glue Schema Registry.
 * It calculates average room temperatures every minute and publish them to another Kafka topic "room-temperatures",
 * again serialized as AVRO using AWS Glue Schema Registry.
 */
public class StreamingJob {

    private static final Logger LOGGER = LoggerFactory.getLogger(StreamingJob.class);

    // Name of the local JSON resource with the application properties in the same format as they are received from the AMSAF runtime
    ...
    // Check if the current execution environment is running locally or not (return false if the app is running on AMSAF)
    ...
    private static Map<String, Properties> loadApplicationProperties(StreamExecutionEnvironment env) throws IOException {
        if (isLocal(env)) {
            ...
            return KinesisAnalyticsRuntime.getApplicationProperties(
                    StreamingJob.class.getClassLoader().getResource(LOCAL_APPLICATION_PROPERTIES_RESOURCE).getPath()
            );
        } else {
            LOGGER.info("Loading application properties from Amazon Managed Service for Flink runtime");
            return KinesisAnalyticsRuntime.getApplicationProperties();
        }
    }
    ...
    private static <T extends SpecificRecord> KafkaSource<T> kafkaSource(
        ...
        return KafkaSource.<T>builder()
                ...
                .setDeserializer(kafkaRecordDeserializationSchema) // https://nightlies.apache.org/flink/flink-docs-release-1.19/docs/connectors/datastream/kafka/#deserializer
    ...
    /**
     * KafkaSink for any AVRO-generated class (SpecificRecord) using AWS Glue Schema Registry
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<TemperatureSample> temperatureSamples = env
                .fromSource(
                        source,
